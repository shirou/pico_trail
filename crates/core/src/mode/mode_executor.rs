...
//! ## Safety
//!
//! - Mode entry failure triggers fallback to previous mode
...
impl ModeExecutor {
    /// Create mode executor with initial mode
    ///
    /// # Arguments
    ///
    /// * `initial_mode` - Initial mode (typically Manual)
    /// * `system_state` - System state for mode reporting
    pub fn new(mut initial_mode: Box<dyn Mode>, system_state: SystemState) -> Self {
        // Enter initial mode
    ...
    /// If new mode entry fails, reverts to previous mode as fallback.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// `Ok(())` on success, `Err` if mode entry fails (fallback to previous attempted)
...
mod tests {
    ...
    #[test]
    fn test_mode_executor_creation() {
        let mode = Box::new(MockMode::new("Test"));
        let system_state = SystemState::new();

        let executor = ModeExecutor::new(mode, system_state);
        assert_eq!(executor.current_mode_name(), "Test");
    }
    ...
    fn test_mode_execute() {
        ...
        let mut executor = ModeExecutor::new(mode, system_state);

        // First execution (timestamp 1_000_000 = 1 second)
        let result = executor.execute(1_000_000);
        assert!(result.is_ok());

        // Second execution (timestamp 1_020_000 = 1.02 seconds, dt = 0.02s)
        let result = executor.execute(1_020_000);
        assert!(result.is_ok());
    }
    ...
    fn test_delta_time_calculation() {
        ...
        executor.execute(1_000_000).unwrap();

        // Second execution at 1.05 seconds (50ms later)
        executor.execute(1_050_000).unwrap();
    }
    ...
    fn test_mode_transition_success() {
        ...
        let mut executor = ModeExecutor::new(initial_mode, system_state);
        assert_eq!(executor.current_mode_name(), "Initial");

        // Transition to new mode
        let new_mode = Box::new(MockMode::new("NewMode"));
        let result = executor.set_mode(new_mode);

        assert!(result.is_ok());
        assert_eq!(executor.current_mode_name(), "NewMode");
    }
    ...
    fn test_mode_transition_entry_failure() {
        ...
        let result = executor.set_mode(failing_mode);
    ...
    fn test_system_state_access() {
        ...
        // Modify system state
        executor.system_state_mut().battery.voltage = 12.5;

        // Read system state
        assert_eq!(executor.system_state().battery.voltage, 12.5);
    }
}
