...
impl<V: VehicleType> MessageDispatcher<V> {
        ...
        match message {
            ...
            COMMAND_LONG(data) => {
                    ...
                    && ack_result == mavlink::common::MavResult::MAV_RESULT_ACCEPTED
                {
                    ...
                    if store
                        .set("COMPASS_DEC", crate::parameters::ParamValue::Float(offset))
                        .is_ok()
                    {
            ...
            MISSION_ITEM_INT(data) => {
                use super::handlers::mission::MissionState;
            ...
            #[allow(deprecated)]
            MISSION_ITEM(data) => {
                use super::handlers::mission::MissionState as TransferState;
                ...
                if matches!(
                    ...
                    TransferState::UploadInProgress { .. }
                ) {
                    crate::log_debug!(
    ...
    ///
    /// Should be called periodically to check if any telemetry messages are
    /// due to be sent based on configured stream rates.
    pub fn update_telemetry(
        ...
        use crate::mission::{take_current_changed, take_item_reached};
        ...
        // Event-driven MISSION_CURRENT on waypoint change
...
mod tests {
    ...
    fn test_dispatch_param_request_list() {
        ...
        // Should return parameter list (may be empty if no params registered)
        // At minimum the dispatch should not panic
        let _ = responses;
    }
    ...
    fn test_dispatch_command_long() {
        ...
        // Should return COMMAND_ACK (+ possible additional messages)
    ...
    fn test_dispatch_heartbeat() {
        ...
        let responses = dispatcher.dispatch(&header, &msg, 1000);

        // Heartbeat should not produce responses
        assert!(responses.is_empty());
        // But should update connection state
        assert!(dispatcher.connection().connected);
        assert_eq!(dispatcher.connection().last_heartbeat_us, 1000);
    }

    #[test]
    fn test_unhandled_message_stats() {
        let mut dispatcher = create_dispatcher();
        ...
        // Send a message type that's not handled by any specific branch
        #[allow(deprecated)]
        let msg = MavMessage::PING(mavlink::common::PING_DATA {
            time_usec: 0,
            seq: 0,
            target_system: 1,
            target_component: 1,
