...
use crate::navigation::geo::calculate_distance;
...
mod tests {
    ...
    fn test_iter_return_path_single() {
        ...
        assert!((p0.latitude - 35.0).abs() < 0.001);
        assert!(iter.next().is_none());
    }
    ...
    fn test_iter_return_path_reverse_order() {
        ...
        let mut iter = recorder.iter_return_path();
        let p0 = iter.next().unwrap();
        assert!((p0.latitude - 0.002).abs() < 0.0001);
        let p1 = iter.next().unwrap();
        assert!((p1.latitude - 0.001).abs() < 0.0001);
        let p2 = iter.next().unwrap();
        assert!((p2.latitude - 0.0).abs() < 0.0001);
        assert!(iter.next().is_none());
    }
