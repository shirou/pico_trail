...
// This is defined in pico_trail_firmware::subsystems::ahrs::AHRS_STATE
...
async fn main(spawner: Spawner) {
    ...
    pico_trail_firmware::log_info!("pico_trail Rover");
    ...
    #[cfg(feature = "pico2_w")]
    {
        ...
        #[cfg(feature = "usb_serial")]
        {
            pico_trail_firmware::log_info!("Initializing USB Serial for debug output...");
            ...
            spawner.spawn(pico_trail_firmware::core::logging::usb_logger_task(cdc_class).unwrap());

            // Spawn USB device task
            spawner.spawn(usb_task(usb).unwrap());

            pico_trail_firmware::log_info!("USB Serial initialized");
        }
        ...
        let in1 = pico_trail_firmware::platform::rp2350::EmbassyPwmPin {
        ...
        pico_trail_firmware::log_info!("Motors initialized");
        ...
        if wifi_config.is_configured() {
            ...
            pico_trail_firmware::log_info!("WiFi configuration loaded:");
            ...
            pico_trail_firmware::log_info!("  NET_SSID - WiFi network name");
            pico_trail_firmware::log_info!("  NET_PASS - WiFi password");
            loop {
    ...
    {
        pico_trail_firmware::log_error!("This example requires pico2_w feature");
        loop {
            Timer::after(Duration::from_secs(60)).await;
        }
    }
    ...
    loop {
        ...
        match read_result {
                ...
                loop {
                    match parser.read_message(&mut reader).await {
                        Ok((header, msg)) => {
                            ...
                            if msg.message_id() != 0 {
                                pico_trail_firmware::log_debug!("RX msgid={}", msg.message_id());
                            }
                        ...
                        Err(_e) => {
                            ...
                            //                            pico_trail_firmware::log_trace!("MAVLink parse finished/error: {:?}", e);
                            break;
                        }
                    }
                }
            ...
            embassy_futures::select::Either::Second(Err(e)) => {
                pico_trail_firmware::log_warn!("Receive error: {:?}", e);
            }
        ...
        critical_section::with(|cs| {
            let mut state =
                pico_trail_firmware::communication::mavlink::state::SYSTEM_STATE.borrow_ref_mut(cs);
            state.update_attitude(&pico_trail_firmware::subsystems::ahrs::AHRS_STATE);
        });
        ...
        for telem_msg in telemetry_messages {
            ...
            if mavlink::write_v2_msg(&mut cursor, header, &telem_msg).is_ok() {
                ...
                if router.send_bytes(&msg_buf[..len]).await.is_ok() {
                    //                    pico_trail_firmware::log_trace!("TX telemetry ({} bytes to all transports)", len);
                    sequence = sequence.wrapping_add(1);
                }
            }
        }
        ...
        if let Some(mission_ack) = dispatcher.check_mission_timeout(timestamp_us) {
            pico_trail_firmware::log_warn!("SENDING MISSION_ACK due to timeout!");
            let header = mavlink::MavHeader {
                system_id: 1,
                component_id: 1,
                sequence,
...
async fn motor_control_task(
    mut motor_group: MotorGroup<
        pico_trail_firmware::libraries::motor_driver::HBridgeMotor<
            pico_trail_firmware::platform::rp2350::EmbassyPwmPin,
            pico_trail_firmware::platform::rp2350::EmbassyPwmPin,
        >,
    >,
) {
...
async fn gps_task(uart: embassy_rp::uart::BufferedUart) {
    use embassy_time::{Duration, Timer};
    use pico_trail_firmware::devices::gps::init::ublox::ubx_commands;
    use pico_trail_firmware::devices::gps::GpsDriver;
    use pico_trail_firmware::devices::gps_operation::GpsOperation;

    pico_trail_firmware::log_info!("GPS task started");
    ...
    pico_trail_firmware::log_info!("GPS: Sending UBX configuration...");
    ...
    pico_trail_firmware::log_info!("GPS UBX initialization complete (GGA/RMC/VTG enabled)");
            ...
            if let (Some(current), Some(target)) = (gps_position, target) {
                ...
                if output.at_target {
                    if mode == FlightMode::Guided {
                        ...
                        complete_mission();
                        pico_trail_firmware::log_info!(
                            "GUIDED: Waypoint reached, mission completed"
                        );
                    } else if mode == FlightMode::Auto {
                        // AUTO mode: advance to next waypoint or complete mission
                        if advance_waypoint() {
                            pico_trail_firmware::log_info!(
                                "AUTO: Waypoint reached, advancing to next"
                            );
                        } else {
                            // Last waypoint reached - mission complete
                            complete_mission();
                            pico_trail_firmware::log_info!(
                                "AUTO: Mission completed, all waypoints reached"
                            );
                        }
                    }
                }
                ...
                NAV_OUTPUT.with_mut(|nav_output| {
                    *nav_output =
                        pico_trail_firmware::subsystems::navigation::NavigationOutput::default();
                });
            }
...
#[cfg(feature = "pico2_w")]
impl pico_trail_firmware::platform::traits::UartInterface for EmbassyBufferedUart {
    fn read(&mut self, buffer: &mut [u8]) -> pico_trail_firmware::platform::Result<usize> {
                ...
                Err(nb::Error::Other(e)) => {
                    ...
                    match e {
                        ...
                        _ => {
                            ...
                            return Err(pico_trail_firmware::platform::PlatformError::Uart(
                                pico_trail_firmware::platform::error::UartError::ReadFailed,
                            ));
                        }
                    }
                }
    ...
    fn write(&mut self, data: &[u8]) -> pico_trail_firmware::platform::Result<usize> {
        use embedded_io::Write;
        self.uart.write(data).map_err(|_| {
            pico_trail_firmware::platform::PlatformError::Uart(
                pico_trail_firmware::platform::error::UartError::WriteFailed,
            )
        })
    }

    fn flush(&mut self) -> pico_trail_firmware::platform::Result<()> {
        use embedded_io::Write;
    ...
    fn set_baud_rate(&mut self, _baud: u32) -> pico_trail_firmware::platform::Result<()> {
        // Embassy BufferedUart doesn't support runtime baud rate changes
        Err(pico_trail_firmware::platform::PlatformError::Uart(
            pico_trail_firmware::platform::error::UartError::InvalidBaudRate,
        ))
    }
...
async fn ahrs_task(
    ...
    use pico_trail_firmware::subsystems::ahrs::Ahrs;

    pico_trail_firmware::log_info!("AHRS task started (BNO086 External AHRS, 100Hz)");

    // Give BNO086 time to stabilize and start sending data
    pico_trail_firmware::log_info!("  Waiting 1s for BNO086 to stabilize...");
    ...
    loop {
        ...
        match ahrs.get_attitude().await {
            Ok(state) => {
                ...
                let timestamp_us = Instant::now().as_micros();
                pico_trail_firmware::subsystems::ahrs::AHRS_STATE.update_euler(
                    state.roll,
                    state.pitch,
                    state.yaw,
                    timestamp_us,
                ...
                {
                    let mut shared_state = pico_trail_firmware::subsystems::ahrs::AHRS_STATE.read();
                    shared_state.angular_rate = state.angular_rate;
                    pico_trail_firmware::subsystems::ahrs::AHRS_STATE.write(shared_state);
                }

                // Mark as healthy after convergence
                if sample_count == CONVERGENCE_SAMPLES {
                    pico_trail_firmware::subsystems::ahrs::AHRS_STATE.set_healthy(true);
                    pico_trail_firmware::log_info!("AHRS converged after {} samples", sample_count);
                }
