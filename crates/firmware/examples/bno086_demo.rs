...
async fn main(#[allow(unused)] spawner: Spawner) {
    ...
    #[cfg(feature = "usb_serial")]
    {
        ...
        pico_trail_firmware::log_info!("USB initialized, waiting for terminal...");
    }

    // Wait 5 seconds at startup to allow serial terminal connection
    pico_trail_firmware::log_info!("Starting in 5 seconds...");
    Timer::after(Duration::from_secs(5)).await;

    pico_trail_firmware::log_info!("BNO086 Demo - GPIO Debug Mode");
    ...
    // =========================================================================
    pico_trail_firmware::log_info!("=== GPIO Pin Test (INT only, RST untouched) ===");
    pico_trail_firmware::log_info!("INT = GPIO22");
    pico_trail_firmware::log_info!("");
    ...
    pico_trail_firmware::log_info!("Initial INT state: {}", int_state);

    // Monitor INT for 2 seconds before I2C init
    pico_trail_firmware::log_info!("Monitoring INT for 2 seconds (before I2C)...");
    ...
    for i in 0..200 {
        ...
        if last_int && !current_int {
            edge_count += 1;
            if edge_count <= 3 {
                pico_trail_firmware::log_info!("  Falling edge #{} at {}ms", edge_count, i * 10);
            }
        }
        last_int = current_int;
    }
    pico_trail_firmware::log_info!("  Edges before I2C: {}", edge_count);
    ...
    // We consume the pins here, then use unsafe steal() to get them back for I2C
    {
        ...
        if sda.is_low() {
            pico_trail_firmware::log_warn!("  SDA stuck low, attempting recovery...");
            ...
            for i in 0..16 {
                ...
                if sda.is_high() {
                    pico_trail_firmware::log_info!("  SDA released after {} clock pulses", i + 1);
                    break;
                }
            }
            ...
            if sda_check.is_high() {
                ...
                pico_trail_firmware::log_warn!(
                    "  SDA still LOW after recovery - may need power cycle"
                );
            }
        } else {
    ...
    // =========================================================================
    pico_trail_firmware::log_info!("Initializing I2C0 (GPIO4=SDA, GPIO5=SCL, 50kHz)...");
    ...
    for attempt in 1..=3 {
        pico_trail_firmware::log_info!("Scanning I2C bus (attempt {}/3)...", attempt);
        ...
        for addr in [0x4B_u8, 0x4A] {
            ...
            {
                Ok(Ok(_)) => {
                    pico_trail_firmware::log_info!("  Found BNO086 at 0x{:02X}", addr);
                    found_addr = Some(addr);
                    break;
                }
                Ok(Err(_)) => {
                    pico_trail_firmware::log_debug!("  0x{:02X}: NACK", addr);
                }
                Err(_) => {
                    pico_trail_firmware::log_warn!("  0x{:02X}: timeout", addr);
                }
            }
        }
        ...
        if attempt < 3 {
            pico_trail_firmware::log_info!("  Not found, waiting 500ms before retry...");
    ...
    let bno_addr = match found_addr {
        Some(addr) => {
            pico_trail_firmware::log_info!("Using BNO086 at address 0x{:02X}", addr);
            addr
        }
        None => {
            pico_trail_firmware::log_error!("BNO086 not found on I2C bus!");
            pico_trail_firmware::log_error!("Check wiring: SDA=GPIO4, SCL=GPIO5, RST=GPIO17");
            pico_trail_firmware::log_error!("Halting.");
            loop {
    ...
    // =========================================================================
    pico_trail_firmware::log_info!("Using POLLING mode (no INT/RST pins)...");

    // Create SHTP I2C transport
    let transport = pico_trail_firmware::communication::shtp::ShtpI2c::new(i2c0, bno_addr);

    // Create BNO086 driver in basic polling mode
    let config = pico_trail_firmware::devices::imu::bno086::Bno086Config::default();
    let mut driver =
        pico_trail_firmware::devices::imu::bno086::Bno086Driver::new(transport, config);

    pico_trail_firmware::log_info!("Reading raw SHTP packets (skip driver init)...");
    pico_trail_firmware::log_info!("BNO086 should output default reports without SET_FEATURE");
    pico_trail_firmware::log_info!("");
    ...
    // Import SHTP types
    use pico_trail_firmware::communication::shtp::{ShtpPacket, ShtpTransport};

    // Read raw packets to see what sensor outputs by default
    pico_trail_firmware::log_info!("=== Raw packet reading (30 packets) ===");
    ...
    for i in 0..30 {
        ...
        match read_result {
            Err(_) => {
                pico_trail_firmware::log_debug!("Pkt {}: timeout", i);
            }
            Ok(Err(e)) => {
                pico_trail_firmware::log_debug!("Pkt {}: {:?}", i, e);
            }
            Ok(Ok(())) => {
                ...
                if packet.channel == 3 && payload.len() > 10 {
                    ...
                    if report_id == 0x05 || report_id == 0x08 {
                        quat_count += 1;
                        pico_trail_firmware::log_info!("  -> Direct quaternion!");
                    }
    ...
    pico_trail_firmware::log_info!("");
    pico_trail_firmware::log_info!("Found {} quaternion reports", quat_count);

    if quat_count == 0 {
        pico_trail_firmware::log_error!("No quaternion data - sensor may need configuration");
        pico_trail_firmware::log_info!("Trying with driver init...");

        // Recreate driver and init
        let config = pico_trail_firmware::devices::imu::bno086::Bno086Config::default();
        let mut driver = pico_trail_firmware::devices::imu::bno086::Bno086Driver::new(shtp, config);

        match driver.init().await {
            Ok(()) => {
                pico_trail_firmware::log_info!("Driver initialized!");
                shtp = driver.release();
            }
            Err(e) => {
                pico_trail_firmware::log_error!("Init failed: {:?}", e);
                loop {
                    Timer::after(Duration::from_secs(60)).await;
                }
            }
        ...
        pico_trail_firmware::log_info!("=== Reading after init (30 packets) ===");
        for i in 0..30 {
            let mut packet = ShtpPacket::<280>::new();
    ...
    pico_trail_firmware::log_info!("");
    pico_trail_firmware::log_info!("Demo complete. Halting.");
    loop {
        Timer::after(Duration::from_secs(60)).await;
    }
}
