...
mod embassy_impl {
    ...
    impl RcInputHandler {
        ...
        pub async fn handle_rc_channels(
            ...
            // Update RC state (uses blocking mutex with critical section)
        ...
        pub async fn handle_rc_channels_override(
            ...
            // Update RC state using PWM normalization (uses blocking mutex with critical section)
            RC_INPUT.with_mut(|rc| {
...
impl RcInputHandler {
    /// Handle RC_CHANNELS message (host tests)
    pub async fn handle_rc_channels(
        &mut self,
        _rc_data: &mavlink::common::RC_CHANNELS_DATA,
        _current_time_us: u64,
    ) {
        // No-op: Host tests don't have global RC state
    }

    /// Handle RC_CHANNELS_OVERRIDE message (host tests)
    pub async fn handle_rc_channels_override(
        &mut self,
