...
impl<'a> CircleMode<'a> {
    ...
    /// Create new Circle mode (host tests)
    #[cfg(not(feature = "embassy"))]
    pub fn new(actuators: &'a mut dyn ActuatorInterface, config: CircleConfig) -> Self {
        Self {
            actuators,
            nav_controller: SimpleNavigationController::new(),
            state: None,
...
impl<'a> Mode for CircleMode<'a> {
    fn enter(&mut self) -> Result<(), &'static str> {
        ...
        #[cfg(not(feature = "embassy"))]
        {
            crate::log_info!("Circle mode entered (host test)");
        }
...
mod tests {
    ...
    fn test_circle_mode_creation() {
        ...
        let config = CircleConfig::default();
        let circle_mode = CircleMode::new(&mut actuators, config);

        assert_eq!(circle_mode.name(), "Circle");
    }

    #[test]
    fn test_circle_mode_enter_exit() {
        let mut steering_pwm = MockPwm::new();
        let mut throttle_pwm = MockPwm::new();
        let mut system_state = SystemState::new();
        system_state.armed = ArmedState::Armed;
        ...
        let config = CircleConfig::default();
        let mut circle_mode = CircleMode::new(&mut actuators, config);

        // Enter should succeed (host test mode)
        assert!(circle_mode.enter().is_ok());
        assert!(circle_mode.exit().is_ok());
    }
    ...
    fn test_calculate_target_clockwise() {
        ...
        circle_mode.state = Some(CircleState {
            center_lat: 0.0,
            center_lon: 0.0,
            radius: 20.0,
            speed: 2.0,
        ...
        // Vehicle at north position on circle
        let (vehicle_lat, vehicle_lon) = offset_position(0.0, 0.0, 20.0, 0.0);
        let current = GpsPosition {
        ...
        let target = circle_mode.calculate_target(&current).unwrap();

        // Target should be to the east (clockwise from north)
        let target_bearing = calculate_bearing(0.0, 0.0, target.latitude, target.longitude);
        assert!(
            target_bearing > 0.0 && target_bearing < 90.0,
            "Expected target bearing 0-90°, got {}°",
            target_bearing
        );
    }
    ...
    fn test_calculate_target_counterclockwise() {
        ...
        let target_bearing = calculate_bearing(0.0, 0.0, target.latitude, target.longitude);
        assert!(
            !(90.0..=270.0).contains(&target_bearing),
            "Expected target bearing 270-360° or 0-90°, got {}°",
            target_bearing
        );
    }
}
