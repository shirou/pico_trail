...
// Provide stub implementations when Embassy is not available
#[cfg(not(feature = "embassy-executor"))]
pub mod stubs {
    //! Stub implementations for when Embassy is not available
    //!
    //! These functions do nothing but allow code to compile without Embassy.

    /// Stub for monitoring_loop_fast
    pub fn monitoring_loop_fast<F>(_check_fn: F)
    where
        F: FnMut(u64) -> Option<super::super::monitoring::FailsafeReason>,
    {
        // No-op without Embassy
    }

    /// Stub for monitoring_loop_medium
    pub fn monitoring_loop_medium<F>(_check_fn: F)
    where
        F: FnMut() -> Option<super::super::monitoring::FailsafeReason>,
    {
        // No-op without Embassy
    }

    /// Stub for monitoring_loop_slow
    pub fn monitoring_loop_slow<F>(_check_fn: F)
    where
        F: FnMut(u64) -> Option<super::super::monitoring::FailsafeReason>,
    {
        // No-op without Embassy
...
mod tests {
    ...
    #[test]
    fn test_stubs_compile() {
        // This test just verifies that the stub code compiles
        #[cfg(not(feature = "embassy-executor"))]
        {
            use stubs::*;
            monitoring_loop_fast(|_| None);
            monitoring_loop_medium(|| None);
            monitoring_loop_slow(|_| None);
        }
    }
}
