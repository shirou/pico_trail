...
- Associated Plan Document:
  - [T-00005-ahrs-dcm-implementation-plan](plan.md)

## Overview

Implement an Attitude and Heading Reference System (AHRS) using Direction Cosine Matrix (DCM) algorithm to provide accurate real-time attitude estimation (roll, pitch, heading) by fusing gyroscope, accelerometer, and magnetometer data. This implementation satisfies FR-00001 and follows the architecture decision documented in ADR-00001.
...
- Current behavior: IMU drivers exist (BMI088, placeholder for MPU series), but no sensor fusion or attitude estimation capability. Task scheduler (FR-00007) provides 100Hz execution slots.
- Pain points: Without AHRS, navigation algorithms cannot determine vehicle orientation, preventing waypoint navigation (FR-00004).
- Constraints:
...
- Related ADRs: [ADR-00001-ahrs-algorithm-selection](../../adr/ADR-00001-ahrs-algorithm-selection.md)
...
- Calibration data persistence (via parameter system from T-00003)
...
1. **Madgwick/Mahony Filter**
   ...
   - Decision: Rejected in ADR-00001 due to insufficient accuracy
...
Per ADR-00001, EKF can be added later as a compile-time feature for Pico 2 W if field testing reveals DCM accuracy is insufficient.
...
- Update `docs/traceability.md`: Link T-00005 to FR-00001 and ADR-00001
