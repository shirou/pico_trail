# T-00039 Break Spin-in-Place Positive Feedback Loop Design
...
Field testing after T-00038 revealed a self-sustaining spin feedback loop. When heading error exceeds 90 degrees, throttle drops to 0 but steering remains at 1.0. `DifferentialDrive::mix(1.0, 0.0)` produces `left=1.0, right=-1.0`, causing maximum-speed spin in place. The resulting vibration corrupts AHRS yaw readings, sustaining the large heading error indefinitely.

This design addresses root causes 6-9 from AN-00023's post-T-00038 field testing analysis with four complementary mitigations.
...
- Context: Rover spins in circles during Guided mode despite T-00038 PD steering, throttle reduction, and hysteresis
- Current behavior:
...
At 50 Hz, this allows max steering change of 0.04 per frame (full range change in 1 second). This was the intended value per T-00038 design but was not set in the implementation default.
