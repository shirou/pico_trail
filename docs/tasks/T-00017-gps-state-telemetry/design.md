# T-00017 GPS State Management and Telemetry Design
...
Integrate GPS position data into the system state and implement real GPS telemetry in MAVLink messages. The GPS driver runs as an async task, updating position at 1-10Hz. Multiple consumers (telemetry handler, navigation, control loop) need thread-safe access to this data. This design extends the existing SystemState pattern (battery, mode, armed) with GPS fields, following ADR-00021.
...
- Related ADRs: ADR-00021 (GPS State Management Pattern)
...
- Total telemetry overhead: < 1ms per cycle (NFR-00068)
...
- [x] Should we use dedicated GPS Mutex vs SystemState Mutex? → Decided: Use SystemState for consistency (ADR-00021)
