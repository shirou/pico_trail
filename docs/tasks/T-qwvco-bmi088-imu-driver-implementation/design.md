...
- Current behavior: No IMU driver currently exists. AHRS task (T-49k7n) is implemented but requires IMU sensor interface.
...
- Related ADRs:
  - [ADR-wjcrn-imu-driver-architecture](../../adr/ADR-wjcrn-imu-driver-architecture.md): Core architecture decision selecting BMI088 + SPI + interrupt
  - [ADR-6twis-ahrs-algorithm-selection](../../adr/ADR-6twis-ahrs-algorithm-selection.md): AHRS algorithm requiring 400Hz IMU data
  - [ADR-oa2qa-platform-abstraction](../../adr/ADR-oa2qa-platform-abstraction.md): Platform abstraction layer design
...
Alternatives were extensively evaluated in ADR-wjcrn-imu-driver-architecture. Summary:
...
2. **MPU6050 + I2C + Polling**
   ...
   - Cons: I2C jitter risk (0.5-2ms on RP2040), fails NFR-3wlo1
