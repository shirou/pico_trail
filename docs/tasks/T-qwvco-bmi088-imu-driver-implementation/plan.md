...
- Assumptions:
  - Platform abstraction layer provides `SpiInterface` trait (ADR-oa2qa)
  - AHRS DCM algorithm is implemented and ready (T-49k7n)
...
- [ ] Confirm ADR-wjcrn-imu-driver-architecture is referenced and approved
- [ ] Note: No legacy IMU driver code exists; this is initial implementation
- [ ] Ensure `SpiInterface` trait from ADR-oa2qa is used for platform abstraction
- [ ] Align with AHRS requirements from T-49k7n (400Hz data feed)
...
- [ ] **AHRS integration**
  - [ ] Update AHRS task (T-49k7n) to use `ImuSensor` trait
