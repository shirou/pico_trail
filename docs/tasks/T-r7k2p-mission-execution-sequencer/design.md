# T-r7k2p Mission Execution Sequencer Design
...
- Context: Auto mode navigates through NAV_WAYPOINT items using MissionStorage as the single source of truth (ADR-2hs12). Three gaps remain: no telemetry, no DO command execution, no mission management.
- Current behavior:
...
Synchronous trait in core crate with event return values. This keeps core zero-cfg while providing the same decoupling as an async event system. The firmware layer handles the async/Embassy integration. Aligns with ADR-0yapl.
...
- ADR-0yapl already documents the architecture decision
