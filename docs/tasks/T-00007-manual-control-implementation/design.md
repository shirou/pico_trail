...
- Constraints:
  - Memory budget: Vehicle layer < 5 KB RAM total (NFR-00011)
  - Real-time: Mode update must complete within 20ms (50 Hz control loop)
  - Safety critical: Actuators must be neutral when disarmed (NFR-00009)
...
- Decision: Rejected in favor of trait-based approach for extensibility (ADR-00013)
...
- Decision: Rejected in favor of actuator abstraction for safety and testability (ADR-00010)
...
- Decision: Deferred to future enhancement, MAVLink RC sufficient for initial implementation (ADR-00011)
