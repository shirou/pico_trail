...
- Associated Plan Document:
  - [T-00002-mavlink-communication-plan](plan.md)
- Related ADRs:
...
- Current behavior: No GCS communication exists. Platform abstraction (T-00004) and task scheduler (T-00001) are complete and provide UART and periodic task infrastructure.
...
- Related ADRs: ADR-00002 selected rust-mavlink crate with custom message handlers
...
- `storage.rs`: Parameter persistence (deferred to FR-00006 task)
...
- **Message Signing**: Deferred to Phase 4 (optional security feature per ADR-00002)
...
- Use platform abstraction UART trait from T-00004
...
1. **Custom MAVLink Implementation**
   ...
   - Decision: Rejected per ADR-00002, rust-mavlink provides faster development and proven correctness
...
2. **C mavlink Library via FFI**
   ...
   - Decision: Rejected per ADR-00002, prefer pure Rust solution
...
- **rust-mavlink**: Chosen for development velocity, correctness, and no_std support (per ADR-00002)
...
- [ ] How to handle parameter storage persistence (flash writes)? → Decision: Defer to FR-00006 task, keep parameters in RAM only for this task
