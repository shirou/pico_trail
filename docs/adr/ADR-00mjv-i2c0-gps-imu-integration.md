# ADR-00mjv I2C0 Multi-Sensor Bus for GPS and IMU Integration
...
**This ADR has been superseded by [ADR-8tp69-uart0-gps-allocation](ADR-8tp69-uart0-gps-allocation.md) due to hardware verification that the GPS module uses UART interface, not I2C.**
...
The pico_trail autopilot requires GPS position data and IMU orientation data for navigation (FR-333ym-gps-waypoint-navigation), but faces severe GPIO pin constraints on the Freenove Standard board. Only GPIO 0, 1, 17, and 22 are available after motor control (GPIO 2-21) and WiFi (GPIO 23-25, 29) allocations.
...
1. **I2C0 Initialization** (FR-2f599-i2c0-multi-sensor-bus):
...
2. **GPS I2C Driver** (FR-qfwhl-gps-i2c-driver):
...
3. **GPS Operation** (FR-3ik7l-gps-operation-data-management):
