...
The pico_trail autopilot requires accurate attitude estimation (roll, pitch, yaw) for navigation and control. The previous ADR (ADR-6twis) selected DCM as the primary algorithm with EKF as an optional upgrade. This decision supersedes that approach, selecting EKF as the primary implementation.
